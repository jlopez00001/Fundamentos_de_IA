...
class PositionWiseFeedForward(nn.Module):
    ...
    def __init__(self, d_model, d_ff):
        super(PositiocWiseFeedForward, self).__int___()
    ...
    #======================
   def forward(self, x):
       return self.fc2(self.relu(self.fcl(x)))
...
class EncodeLayer(nn.Module):
    ...
    def __init__(self, d_model, num_heads, d_ff, dropout):
        ...
        self.self_attn = MultiHeadAttetion(d_model, num_heads)
...
class DecoderLayer(nn.Module):
    ...
    #===================================
    def __init__(self, d_model, num_heads, d_ff, dropout):
        super(DecodreLayer, self).__init__()
        self.self_attn = MultiHeadAttetion(d_model, num_heads)
        self.cross_attn = MultiHeadAttetion(d_model, num_heads)
        self.feed_forward = PositionWiseFeedForward(d_model, d_ff)
        self.norm1 = nn.LayerNorm(d_model)
        self.norm2 = nn.LayerNorm(d_model)
    ...
    def forward(self, x, enc_output, src_mask, tgt_mask):
        ...
        x = self.norm1(x + self.dropout(attn_outpu))
        attn_output = self.cross_attn(x, enc_output, enc_output, src_mask)
        x = self.norm2(x + self.dropout(attn_outpu))
        ff_output = self.feed_forward(x)
        x = self.norm3(x + self.dropout(ff_outpu))
        return x
...
class Transformer(nn.Module):
    ...
    #=================================
    def __init__(self, d_model, num_heads, d_ff, dropoutsrc_vocab_size, tgt_vocab_size, max_seq_length, num_layers):
    ...
    #========================
    #  Algoritmo Tranformer
    #========================
    def forward(self, src, tgt):
        src_mask, tgt_mask = self.generate_mask(src, tgt)
        src_embedded = self.dropout(self.positional_ecoding(self.encoder_embedding(src)))
        tgt_embedded = self.dropout(self.positional_ecoding(self.encoder_embedding(tgt)))
...
#=========================
transformer = Transformer(src_vovab_size,tgt_vocab_size,d_model,num_heads,num_layers,d_ff,max_seq_lengthdropout)
...
#===============================================
optimizer = optim.Adam(transformer.parameters(),lr=0.0001,betas=(0.9, 0.98),eps=le-9)
